...
google_credentials = service_account.Credentials.from_service_account_file("D:\\Vqode\\cogent-silicon-447020-a9-e831ace88f34.json")
storage_client = storage.Client(credentials=google_credentials)

bucket_name = "brij_automation_status"
allure_report_path = os.path.join(os.path.abspath(__file__+"/../../"), "Report\\allure_report")
allure_report_cloud_path = f"allure_reports/allure_{datetime.now().strftime('%d_%m_%Y_%I_%M_%p')}"
...
def delete_allure_result_and_failed_screenshots():
    ...
    screenshot_folder = os.path.join(os.path.abspath(__file__+"/../../"), "Screenshots")
